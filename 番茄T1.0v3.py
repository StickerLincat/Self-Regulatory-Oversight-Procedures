...
""".format(os.path.abspath(__file__))
    ...
    def load_config(self):
        try:
            with open(CONFIG_FILE, 'r') as f:
                ...
                self.process_blacklist = data.get('blacklist', [])
        ...
        while True:
            if self.is_in_restricted_period():
                for proc in psutil.process_iter(['name']):
                    ...
                    for blacklist_item in self.process_blacklist:
    ...
    def add_supervision_item(self):
        ...
        ttk.Combobox(add_win, textvariable=action_var, values=["关机", "锁定", "提醒"]).grid(row=3, column=1)
    ...
    def add_blacklist_process(self):
        ...
        if proc and proc not in [item['name'] for item in self.process_blacklist]:
            self.process_blacklist.append({"name": proc, "active": True})
            self.save_config()
            self.refresh_blacklist()
    ...
    def refresh_blacklist(self):
        ...
        for idx, proc in enumerate(self.process_blacklist):
            ...
            chk_btn = ttk.Checkbutton(
                frame,
                variable=chk_var,
                command=lambda i=idx, var=chk_var: self.toggle_blacklist(i, var)
            )
            chk_btn.pack(side=LEFT)
            if is_locked:
                chk_btn.state(['disabled'])
            ...
            btn = ttk.Button(frame, text="删除", command=lambda i=idx: self.delete_process(i))
            if is_locked:
                btn.state(['disabled'])
            btn.pack(side=RIGHT)

    def toggle_blacklist(self, index, check_var):
        if self.is_in_restricted_period():
            check_var.set(not check_var.get())
            messagebox.showwarning("操作受限", "当前处于或即将进入监督时段，无法修改黑名单状态！")
            return
            
        self.process_blacklist[index]['active'] = check_var.get()
        self.save_config()
